...
// import { detectAndRegisterSolflareMetaMaskWallet } from './metamask/detect'

export interface SolflareProvider extends BaseProvider {
  isSolflare?: boolean
}
...
export class SolflareAdapter extends BaseAdapter<SolflareProvider> {
  ...
  checkProvider = ({ provider }: { provider?: SolflareProvider }) => {
    return window.solflare?.isSolflare || window.SolflareApp
  }

  getProvider = () => ({ provider: (window.solflare || window.SolflareApp) as SolflareProvider })
